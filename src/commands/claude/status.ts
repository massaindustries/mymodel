...
export default class ClaudeStatus extends Command {
  ...
  static flags = {
    url: Flags.string({
      char: 'u',
      description: 'mymodel base URL (default: $ANTHROPIC_BASE_URL or http://localhost:18000)',
    }),
  };
  ...
  async run(): Promise<void> {
    ...
    this.log(`  mymodel             ${baseUrl}  ${health ? `${tick} healthy` : `${cross} unreachable`}`);

    const diag = await fetchDiag(baseUrl);
    if (!diag) {
      this.log(`  classifier          ${cross} could not reach /api/v1/diag/classifier on mymodel`);
    ...
    if (!health) {
      this.log('');
      this.log(`${COLORS.yellow}mymodel is not responding. Skipping routing stats.${COLORS.reset}`);
      return;
    }
    ...
    if (!metrics) {
      ...
      this.log(`${COLORS.dim}  (mymodel exposes Prometheus metrics on its own port — usually 9190 or 19190.)${COLORS.reset}`);
      return;
    }
