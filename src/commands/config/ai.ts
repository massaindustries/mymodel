...
export default class ConfigAi extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!process.stdin.isTTY) {
      this.error('mymodel config <name> ai requires an interactive TTY.', { exit: 2 });
    }
    ...
    if (state.runningProfile) {
      ...
      if (!apiKey) {
        err(`no profile running and no REGOLO_API_KEY in ${pp.env}. Either run \`mymodel serve <name>\` first or set the key.`);
        this.exit(1);
      }
