...
export default class Serve extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (state.runningProfile && state.runningProfile !== profile) {
      err(`profile '${state.runningProfile}' is currently running. run \`mymodel stop ${state.runningProfile}\` first.`);
      this.exit(1);
    }

    const pp = paths(profile);
    try { await stat(pp.config); } catch { err(`no config at ${pp.config}. run \`mymodel config new ${profile}\` first.`); this.exit(1); }
    try { await stat(pp.compose); } catch { err(`no compose at ${pp.compose}. run \`mymodel config new ${profile}\` first.`); this.exit(1); }
