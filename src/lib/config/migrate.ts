...
/**
 * Migrate a legacy `~/.mymodel/{config.yaml,docker-compose.yml,.env,models/}`
 * layout into `~/.mymodel/profiles/default/`. Idempotent: runs at most once
...
export async function migrateLegacyLayout(): Promise<{ migrated: boolean; reason?: string }> {
  ...
  // Compose was rendered with legacy ~/.mymodel/config.yaml + ~/.mymodel/.env paths.
