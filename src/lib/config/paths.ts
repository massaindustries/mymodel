 ...
 * Order: explicit arg → MYMODEL_PROFILE env → state.activeProfile → throw with guidance.
 */
export function resolveProfile(explicit?: string): string {
  const candidate = explicit ?? process.env.MYMODEL_PROFILE ?? readState().activeProfile;
  if (!candidate) {
    const profs = listProfiles();
    if (profs.length === 0) {
      throw new Error('no profiles found. Run `mymodel config new <name>` (or `mymodel init`) to create one.');
    }
    throw new Error(`no active profile. Run \`mymodel config use <name>\` or pass --profile. Available: ${profs.join(', ')}`);
  }
  if (!profileExists(candidate)) {
    throw new Error(`profile '${candidate}' not found. Run \`mymodel config list\` to see available profiles.`);
  }
  return candidate;
}
